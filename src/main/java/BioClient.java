...
public class BioClient {
    ...
    private static void 连接心跳服务() {
        new Thread(BioClient::进行心跳连接, "心跳处理-基础线程").start();
    }
            ...
            case "addThread":
                ...
                synchronized (LOCK) {
                    if (本地代理线程池.getMaximumPoolSize() - 本地代理线程池.getActiveCount() <= 3) {
                        本地代理线程池.setMaximumPoolSize(Math.min(本地代理线程池.getMaximumPoolSize() + Env.默认的客户端代理_增加数量, 200));
                    }
                }
    ...
    private static void 创建代理服务器资源() {
        try {
            本地代理线程池.execute(BioClient::连接远程代理服务器);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
